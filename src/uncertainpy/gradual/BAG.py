    ...
    def remove_argument(self, arg):
        ...
        for i in range(len(self.supports)-1, -1, -1): # only consider acyclic case so only two if
            ...
            if self.supports[i].get_supporter()==self.arguments[arg]:
                ...
                # delete supporter
                # note that remove supporter does not mean delete the supporter argument in the QBAF,
                # it only means delete the supporter argument in the current argument's supporter list
